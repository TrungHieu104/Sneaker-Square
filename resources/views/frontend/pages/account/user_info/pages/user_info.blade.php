...
@push('css-access')
    <style>
        /* `.img-user` is width:100% of its column, which on this page meant a
           450px portrait; the edit button hung outside the ring for want of a
           vertical anchor. */
        .profile-avatar {
            width: 140px;
        }
...
<div class="bg-white container-fluid p-md-5 p-3 page-user-info"
    ...
    <form action="{{ route('thong-tin-tai-khoan.update', [Auth::guard('web')->user()->user_id]) }}"
        ...
        <div class="d-flex flex-column align-items-center mb-4">
            <div class="img-user profile-avatar">
                <img id="previewImage" onerror="this.src='/uploads/img_error3.png'" src="{{ $user->user_img }}" alt="">
                <a class="edit-button" title="Đổi ảnh đại diện">
                    <i class='bx bxs-edit-alt'></i>
                </a>
                <input type="file" name="img__new" id="fileInput" class="files" style="display: none;">
            </div>
            @if ($errors->has('img__new'))
                @foreach ($errors->get('img__new') as $error)
                    <small class="text-danger fst-italic mt-2">{{ $error }}</small>
                @endforeach
            @endif
        </div>
        ...
        <div class="row">
            <div class="col-lg-6 mb-3">
                ...
                <input value="{{ $user->name }}" name="name" id="fname" class="form-control" type="text">
                @if ($errors->has('name'))
            ...
            <div class="col-lg-6 mb-3">
                ...
                @if ($user->email_verified_at === null)
                    <small class="fst-italic" style="color: var(--color-red);">Email chưa được xác minh.</small>
                @endif
            </div>
            <div class="col-lg-6 mb-3">
                <label class="form-label" for="uphone">Số điện thoại</label>
                <input class="form-control" name="user_phone" id="uphone"
                    placeholder="{{ $user->user_phone ? $user->user_phone : 'Chưa cập nhật số điện thoại' }}"
            ...
            <div class="col-lg-6 mb-3">
                ...
                <input class="form-control" id="uaddress" disabled type="text"
                    value="{{ $address ? $address->info_address.', '.$address->info_ward.', '.$address->info_district.', '.$address->info_province : 'Chưa có địa chỉ nhận hàng' }}">
        ...
        <div class="row align-items-center g-3">
            <div class="col-lg-5 col-12 d-flex align-items-center">
                ...
                <span class="ms-3">Tài khoản Google:
                    <span class="text-primary">{{ $user->google_id ? 'Đã liên kết' : 'Chưa liên kết' }}</span>
                </span>
            </div>
            <div class="col-lg-7 col-12 d-flex flex-wrap gap-2 justify-content-lg-end">
                <button type="button" class="border grey-hover border-1 custom-btn text-dark"
                    data-bs-toggle="modal" data-bs-target="#changeEmail">Thay đổi Gmail</button>
                @if ($user->email_verified_at === null)
                    <button type="button" class="border grey-hover border-1 custom-btn text-dark"
                        data-bs-toggle="modal" data-bs-target="#verifiedEmail">Xác minh email</button>
                @endif
