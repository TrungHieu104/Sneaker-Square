{{--
    The customer's request to send a completed order back, and whatever the
    shop can do about it at this step.

    Rendered outside the order form: every action here is a form of its own.

    @param \App\Models\OrderModel $order
    @param \App\Models\OrderReturnModel $return
--}}
@php
    ...
    <div class="card-body">
        <div class="d-flex align-items-center justify-content-between mb-3">
            ...
            <div class="d-flex gap-2">
                <span class="badge bg-label-{{ $traMotPhan ? 'warning' : 'dark' }} fs-6">{{ $traMotPhan ? 'Trả một phần' : 'Trả toàn bộ' }}</span>
                <span class="badge bg-label-{{ $mauTrangThai[$return->status] ?? 'secondary' }} fs-6">{{ $return->statusLabel() }}</span>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-lg-6 mb-3">
                ...
                @endif
                <div class="mb-2"><b>Thông tin nhận tiền hoàn:</b> {{ $return->refund_info }}</div>
                ...
                <div class="mb-2 text-muted small">Gửi lúc {{ $return->created_at->format('H:i d/m/Y') }}</div>
